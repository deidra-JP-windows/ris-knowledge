...
# サービスレベル/SLI/SLA/SLO/メトリクス/エラーバジェット/バーンレート
- サービスレベル
  - 抽象的な品質概念
    - 一定以上サービスが動いていることなど
- SLI
	- サービスレベルを測定する指標
	  - 抽象的な概念（一定以上サービスが動いていることなど）を測定可能な数値を基に計算し表したもの
    - SLA/SLO を計算する方法を含む定義でもある
      - 指標になる定義
	- リクエストの成功率とか
	  - Valid,Good,Badで考える
	    - Valid
		...
		- Good
		  - 成功したリクエスト数
		    - (2%のみだったり 2%+5% or 2%+4%だったり)
			  - サービスによって担保される責任分界点によってかわる
			    - 5%はAWSガワなんでBad にしなくていいとか
		- Bad
		  - 失敗したリクエスト数
		    - 4% のみとか 5%とか
	  - % で表せる形になる
	- メトリクスやログ、トレース、ダンプなどのシグナルから作成する
- SLI測定値
  - 特定地点でのSLIの測定値
    - ~%とかででる
- SLA
  - サービスレベルについて握った稼働率であり契約
    - SLI測定値で99.9とか
- SLO
  - サービスレベルについて運用側での目標値
  - SLI測定値で見たときに SLA=99.9 であれば 99.95とか
- メトリクス
  - 何かしらの活動や状態を定量的に可視化する為のデータ
    - サービスレベルやを計算するために使われる
    - Valid / good / bad のメトリクスを用いて、サービスレベル / エラーバジェット / バーンレート　を計算する
- エラーバジェット
  - 以下の4点で考える
    - エラーバジェット比率
	  - エラーバジェット比率 = 1 - SLO
    - 許容エラー数
	  - 許容エラー数 = エラーバジェット比率 × リクエスト数
	- 残りエラーバジェット
	  - 残りエラーバジェット = 許容エラー数 - エラー数
	- エラーバジェット消費率
	  - エラーバジェット消費率 = エラー数 / 許容エラー数
	  - エラーバジェットを可視化する際にはここを出すケースが多い
	    - 閾値切ったら赤
- バーンレート
  - 今のエラー発生ペースが続いたら、エラーバジェットをどれくらいの速さで使い切るかという倍率
  - 失敗率 = 1 - 成功率
  - バーンレート = 失敗率 / 許容エラー数

# REDメソッド
  - 主要エンドポイントの「レート・エラー・遅延」でサービスの健康を見る指標セット。
    - レート
	  - リクエストレート
	    - 1秒あたりのリクエスト数など
	- エラー
	- 遅延
# 4大シグナル
  - 「レイテンシ・トラフィック・エラー・飽和」でシステム状態を監視する基本指標
# クリティカルユーザジャーニー
- CUJ
- ユーザーにとって最重要な目的達成フローに絞ったユーザージャーニー。
# ユーザジャーニー
- ユーザーが目的を達成するまでの体験の流れ（タスクの一連の道筋）。